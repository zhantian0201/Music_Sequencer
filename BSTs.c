...
BST_Node *newBST_Node(double freq, int bar, double index)
{
    ...
    if (new_node == NULL)
	{
		printf("Error with creating a new node");
		return NULL;
	}
	new_node->freq = freq;
	new_node->bar = bar;
...
BST_Node *BST_insert(BST_Node *root, BST_Node *new_node)
{
	...
	if (new_node->index < 0.0||new_node->index > 1.0 || new_node->bar < 0)
	{
		printf("Bar or index: Out of range!\n");
		free(new_node);
		return root;
	}
...
void findMinMaxBars(BST_Node *root, int *minBar, int *maxBar) 
{
    if (root == NULL) return;
    if (root->bar < *minBar) *minBar = root->bar;
    if (root->bar > *maxBar) *maxBar = root->bar;
    findMinMaxBars(root->left, minBar, maxBar);
    findMinMaxBars(root->right, minBar, maxBar);
}

BST_Node *changeBarIndex(BST_Node *root, int minBar, int maxBar, BST_Node *new_root) {
    if (root == NULL) return new_root;
    int new_bar = maxBar + minBar - root->bar;
    double new_index = 1.0 - root->index;
    BST_Node *new_node = newBST_Node(root->freq, new_bar, new_index);
	if (new_node == NULL) 
	{
    ...
    new_root = changeBarIndex(root->left, minBar, maxBar, new_root);
    new_root = changeBarIndex(root->right, minBar, maxBar, new_root);
    return new_root;
}
...
BST_Node *reverseSong(BST_Node *root)
{
    ...
    int minBar = root->bar;
    int maxBar = root->bar;
    findMinMaxBars(root, &minBar, &maxBar);
    BST_Node *new_root = NULL;
    new_root = changeBarIndex(root, minBar, maxBar, new_root);
    return new_root;
}
...
int findFreqIndex(double freq)
{
	...
	printf("No such freq in the musical scale!\n");
	return -1;
}
int BST_count(BST_Node *root) {
    if (root == NULL) return 0;
    return 1 + BST_count(root->left) + BST_count(root->right);
}

void collectNotes(BST_Node *root, BST_Node **notes, int *count) 
{
    if (root == NULL) return;
    collectNotes(root->left, notes, count);
    notes[*count] = root;
    (*count)++;
    collectNotes(root->right, notes, count);
}
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
	...
	if (root==NULL) return NULL;
	BST_Node **notes = malloc(BST_count(root) * sizeof(BST_Node *));
	if (notes==NULL)
	{
		printf("Memory allocation failed for notes array.\n");
		return NULL;
	}
	int note_count = 0;
    collectNotes(root, notes, &note_count);
	for (int i = 0; i < note_count; i++) 
	{
		BST_Node *original_note = notes[i];
		int flag = 1;
		int old_freq_index = findFreqIndex(original_note->freq);
		if (old_freq_index==-1)
		{
			flag = 0;
		}
		int new_freq_index = old_freq_index+semitones;
		if (new_freq_index<0 || new_freq_index>99)
		{
			flag = 0;
		}
		double new_freq = note_freq[new_freq_index];
		double new_index = original_note->index+time_shift;
		...
		if (flag == 1) 
		{
            BST_Node *new_node = newBST_Node(new_freq, original_note->bar, new_index);
			...
			{
                BST_Node *new_root = BST_insert(root, new_node);
                if (new_root == NULL) 
				{
                    free(new_node);
                }
